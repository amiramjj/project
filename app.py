...
def blueprint_score(row):
    ...
    # ----- BONUSES (nice-to-have, unless client requested) -----
    if row.get("maidpref_smoking") == "non_smoker":
...
# Streamlit App
# -------------------------------
st.title("Client–Maid Matching Score (Blueprint Scoring)")

uploaded_file = st.file_uploader("Upload your dataset (Excel)", type=["xlsx", "csv"])
...
if uploaded_file:
    ...
    st.write("### Scoring Results", results.head(50))

    # Option to download full results
    csv = results.to_csv(index=False).encode("utf-8")
    st.download_button("Download Full Scoring Results", csv, "scoring_results.csv", "text/csv")
